...
const TodoList = () => { // Убрали параметр (todo) - он не нужен
	...
	const handleAddTodo = async (newTodo) => {
		try {// Отправляем POST запрос на JSON Server
			const response = await fetch('http://localhost:3000/todos', {
				method: 'POST',
				headers: {
	...
	// Функция для обновления задачи (PUT/PATCH)
	const handleUpdateTodo = async (id, updatedData) => {
		try {
			...
			console.error('Ошибка:', error);
			setOperationError('Не удалось обновить задачу');
			// Автоочистка через 3 секунды
			setTimeout(() => setOperationError(null), 3000);
			throw error; // Пробрасываем ошибку для обработки в TodoItem
		}
	};

	// Функция для удаления задачи (DELETE)
	const handleDeleteTodo = async (id) => {
		try {
			...
			// Обновляем состояние - удаляем задачу из списка
			setTodos(prevTodos => prevTodos.filter(todo => todo.id !== id));
			setOperationError(null); // Сбрасываем ошибку операций

			console.log('Задача удалена, ID:', id);
		} catch (error) {
			console.error('Ошибка:', error);
			setOperationError('Не удалось удалить задачу');
			// Автоочистка через 3 секунды
			setTimeout(() => setOperationError(null), 3000);
			throw error; // Пробрасываем ошибку для обработки в TodoItem
		}
			...
			{/* <div className="todo-stats">
				...
				{searchTerm && (
					<div className="search-stats">
						Показано: {filteredTodos.length} из {todos.length}
					</div>
				)}
			</div> */}
						...
						<div className="empty-state">
							{searchTerm ? (
								<>
									...
									>
										Очистить поиск  Показать все задачи
									</button>
								</>
							) : (
